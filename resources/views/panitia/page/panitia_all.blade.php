...
@section('additional-footer')
    ...
    <script type="application/javascript">
        $(document).ready(function(){function b(a){var b="{{auth('panitia')->user()->role_id}}",c='<button class="btn btn-primary btn-flat btn-sm detailbutton" data-toggle="tooltip" data-placement="top" title="lihat data secara detail"><span class="ion ion-navicon-round"></span></button>',d="",e="";if("3"!=b&&1!=a.posisi_id)var d='<a class="btn btn-warning btn-sm btn-flat resetbutton" role="button" data-toggle="tooltip" data-placement="right" title="Reset Password Peserta"><span class="ion ion-ios-refresh"></span></a>',e='<a class="btn btn-danger btn-sm btn-flat deletebutton" role="button" data-toggle="tooltip" data-placement="right" title="Delete Peserta"><span class="ion ion-ios-trash"></span></a>';return c+d+e}var a=$("#data_peserta").DataTable({ajax:{headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")},url:"{{ route('panitia_data_panitia_all') }}",type:"post"},Processing:!0,ServerSide:!0,columns:[{data:"username",width:"15%",render:function(a){return"<h6>"+a+"<h6>"}},{data:"nama_lengkap",width:"20%",render:function(a){return"<h6>"+a+"</h6>"}},{data:"nomor_telepon",render:function(a){return"<h6>"+a+"</h6>"}},{data:"panlok",render:function(a){return"<h6>"+a+"</h6>"}},{data:null,width:"15%",render:function(a){return"<h6>"+b(a)+"</h6>"}}]});$("#data_peserta tbody").on("click",".detailbutton",function(){var b=a.row($(this).parents("tr")).data();$("#nama_lengkap").text(b.nama_lengkap),$("#username").text(b.username),$("#nomor_telepon").text(b.nomor_telepon),$("#panlok").text(b.panlok),$("#posisi").text(b.posisi),$("#modal-detail").modal("show")}),$("#data_peserta tbody").on("click",".resetbutton",function(){var b=a.row($(this).parents("tr")).data();return!!confirm("Apakah anda yakin akan mereset password dari panitia "+b.username+" ?")&&void $.ajax({headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")},type:"POST",url:"{{ route('panitia_reset_password_panitia') }}",data:"username="+b.username}).done(function(a){alert(a)})}),$("#data_peserta tbody").on("click",".deletebutton",function(){var b=a.row($(this).parents("tr")).data();return!!confirm("Apakah anda yakin akan menghapus user dari panitia "+b.username+" ?")&&void $.ajax({headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")},type:"POST",url:"{{ route('panitia_delete_panitia') }}",data:"username="+b.username}).done(function(b){a.ajax.reload(null,!1),alert(b)})})});
    </script>
